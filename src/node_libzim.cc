...
NAN_METHOD(ArticleSourceWrap::getMainPage) { WRAPPER_GET_STRING(getMainPage); }
NAN_METHOD(ArticleSourceWrap::getLayoutPage) { WRAPPER_GET_STRING(getLayoutPage); }
...
NAN_METHOD(ZimCreatorWrap::setMinChunkSize) {
  ...
  getWrappedField(info)->setMinChunkSize((int) size);
  info.GetReturnValue().Set(Nan::Undefined());
}
